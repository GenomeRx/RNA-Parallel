...
matrix_backends <- function(env = parent.frame()) {
  # parent.frame() is a lazy default. Forced later, inside the backend closure, it resolves
  # against a frame that has already returned and lands on globalenv(), where the defer below
  # only fires at session exit and every run leaks its workers. Force it while the caller
  # still exists.
  force(env)
  ...
  # One cluster for the whole sweep, built on first use rather than per dispatch. Standing a
  # fresh PSOCK cluster up on every dispatch broke a worker connection mid-send about one run
  # in five: serialize(data, node$con) on a socket the previous cluster had just torn down.
  # Cluster size does not touch the result. The chunk split is decided before the backend is
  # called, so a backend only maps over blocks it is handed and parLapply preserves order.
  # Sized from the core cap alone so it cannot depend on which dispatch arrives first, and
  # fixed at first use.
  cl <- NULL
  b[["parLapply_PSOCK"]] <- function(idx, f, w) {
    if (is.null(cl)) {
      cl <<- parallel::makeCluster(test_max_cores(), type = "PSOCK")
      # cleanup registered before anything that can throw: clusterEvalQ propagates worker
      # errors, and a failure there would otherwise strand a live cluster with cl non-NULL
