...
#' @param workers Maximum concurrent worker processes. 4 is the default because 6
#'   has kernel-panicked a 24 GB machine when a second R process was also forking.
#' @param chunks Row chunks per parallel step. Defaults to `workers`. More chunks
#'   means smaller peak memory per worker and slightly more fork overhead.
...
ComBat_seq_parallel <- function(counts, batch, group = NULL, covar_mod = NULL,
                                ...
                                gene.subset.n = NULL, workers = 4L, chunks = NULL,
