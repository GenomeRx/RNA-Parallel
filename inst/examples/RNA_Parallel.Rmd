...
| C. Simulated counts | ComBat-seq then limma and edgeR, each against its original, at 2, 4, 6 and 8 workers |
...
# than assumes.
#
# MACOS SWEEP. 2, 4, 6, 8. Every platform's sweep brackets its PERFORMANCE core count, which is
# not always its core count. This is an Apple M3: 8 physical cores, 4 performance and 4
# efficiency, and 8 logical, since none of them carries a second thread. Unlike the socket
# backends the other platforms fall back to, a forked worker on an efficiency core still adds
# throughput, because it shares the parent's pages instead of receiving a copy. Forks also
# migrate across the two clusters, so no worker is stranded on a slow one, and the 8-worker arm
# is measured rather than assumed to be worse.
#
# The Linux run used 4, 8, 16, 32 on a dual Xeon whose 16 cores all carry SMT, and the Windows
# run used 2, 4, 6, 8 on an Ultra 9 185H with 6 performance cores. Same rule, three answers.
...
# BACKEND. mclapply, which is the package default wherever fork() exists. A forked worker reads
# the matrix copy-on-write, so a large voom object costs nothing to distribute; the socket
# backends must re-serialise it per chunk and measured slower than not parallelising at all.
# Set as an option rather than left implicit so every companion call inherits the same one and
# the machine table below reports what actually ran.
options(combat.backend = "mclapply")
options(future.globals.maxSize = 8 * 1024^3)

# Accelerate is the system BLAS here and manages its own thread pool, so there is no
# OPENBLAS_NUM_THREADS to pin as there is on Linux. A user-installed OpenBLAS or MKL build is
# multi-threaded and would open a pool inside every forked worker on top of the fork, so the
# thread count is reported either way rather than assumed.
# sessionInfo()$BLAS is NULL on an R built against its own internal BLAS, and basename(NULL)
# does not return character(0), it errors: "a character vector argument expected". That is in
# this chunk, so such a machine would stop in the render's first seconds. All three reports
...
blas_thread <- Sys.getenv("OPENBLAS_NUM_THREADS", Sys.getenv("VECLIB_MAXIMUM_THREADS", ""))
...
# Reported, not assumed. Every note in this file is about a platform that has fork(), so a run
# on one that does not would be describing something that is not happening.
on_macos <- identical(Sys.info()[["sysname"]], "Darwin")
if (!on_macos)
  warning("This is the macOS report and this is not macOS. Section A describes fork() and the ",
          "performance/efficiency split of an Apple M3. Render RNA_Parallel_linux.Rmd or ",
...
```{r machine}
# The same thirteen rows as the Linux and Windows reports, read from this platform's own
...
sysctl_num <- function(k) suppressWarnings(as.numeric(sysctl(k)))

n_log  <- parallel::detectCores()
n_phys <- suppressWarnings(parallel::detectCores(logical = FALSE))
if (is.na(n_phys) || n_phys < 1L) n_phys <- n_log
# Apple silicon reports its performance cores directly, which no other platform here does.
n_perf <- sysctl_num("hw.perflevel0.physicalcpu")
if (is.na(n_perf) || n_perf < 1) n_perf <- n_phys
mem_gb <- sysctl_num("hw.memsize") / 2^30
...
data.frame(
  ...
  value = as.character(c(
    if (on_macos) sysctl("machdep.cpu.brand_string") else NA_character_,
    n_perf,
    n_phys,
    n_log,
...
All three worker counts are shown, matching every other sweep in the report. Eight workers do not
return twice what four return, and not because half of them are stranded on efficiency cores:
given identical work, eight forked children here finish within 1.08x of each other, where
stranding would show as roughly threefold. Concurrency is what costs, so each added worker
returns less throughput than the one before it.
...
ggplot(all_speed, aes(factor(workers), speedup, fill = stage)) +
  ...
  labs(x = "workers", y = "speedup against the original", fill = NULL,
       ...
       subtitle = "Dashed line is the original. Every arm at 2, 4, 6 and 8 workers.") +
  expand_limits(y = 0) +
  theme(legend.position = "bottom")
```
