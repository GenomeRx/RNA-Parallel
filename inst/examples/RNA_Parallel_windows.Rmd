...
| C. Simulated counts | ComBat-seq then limma and edgeR, each against its original, at 2, 4, 8 and 16 workers, across every backend |
...
# workers is run as asked: 16 means 16 worker processes, bounded only by detectCores(). Added
# workers return less throughput each, so whether 16 beats 8 is what the sweep measures rather
# than assumes.
#
# WINDOWS SWEEP. 2, 4, 8, 16. The macOS run used 2, 4, 6, 8 on an 8-core M3 and the Linux run
# used 4, 8, 16, 32 on a 16-core dual Xeon. This is a third machine and the sweep is set for it
...
#                 future in this process. Every future arm below sets multisession first.
#   foreach       real parallelism over PSOCK, and it needs nothing from the caller: the package
#                 registers its own cluster when no backend is registered. This is what
#                 mclapply's own fallback message recommends here.
#
...
if (length(absent_backend))
  stop("the backend sweep in C.1.a needs these packages and they are not installed: ",
       ...
       call. = FALSE)
options(combat.backend = "foreach")
...
sim_timings <- do.call(rbind, lapply(seq_len(nrow(sim_grid)), function(i) {
  ...
  r <- callr::r(function(f, b, w) {
    ...
    # future is the one backend whose parallelism the caller owns. The package refuses to touch
    # a session's plan, so without this the arm resolves every future in this process and times
    # as serial while the table calls it parallel.
    if (b == "future") {
...
sim_par <- sim_timings[sim_timings$backend != "serial", ]
sim_par$backend <- factor(sim_par$backend, levels = win_backends)
sim_best <- sim_par[which.max(sim_par$speedup), ]

ggplot(sim_par, aes(factor(workers), speedup)) +
...
data.frame(
  quantity = c("CPU", "physical cores", "logical CPUs", "threads per core", "memory", "OS",
               "BLAS", "BLAS pinned to 1 thread", "fork available", "backend", "worker counts"),
