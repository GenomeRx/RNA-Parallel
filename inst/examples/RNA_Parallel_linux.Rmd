...
| C. Simulated counts | ComBat-seq then limma and edgeR, each against its original, at 4, 8, 16 and 32 workers |
...
# workers is run as asked: 32 means 32 forked processes, bounded only by detectCores(). Added
# workers return less throughput each, so whether 32 beats 16 is what the sweep measures rather
# than assumes.
#
...
# sweep is 4, 8, 16, 32, where 16 is every physical core and 32 is every hyperthread. The
# 16 -> 32 step adds no arithmetic units, only a second thread per core, and is read that way.
# TCGA is corrected at every count; the last arm is the one the figures below use.
tcga_workers <- c(4L, 8L, 16L, 32L)
...
here and this sweep extends to 32 instead of stopping at 8. Concurrency is what costs, so each
added worker returns less throughput than the one before it, and the 16 to 32 step is where
...
ggplot(all_speed, aes(factor(workers), speedup, fill = stage)) +
  ...
  labs(x = "workers", y = "speedup against the original", fill = NULL,
       ...
       subtitle = "Dashed line is the original. Every arm at 4, 8, 16 and 32 workers.") +
  expand_limits(y = 0) +
  theme(legend.position = "bottom")
```
