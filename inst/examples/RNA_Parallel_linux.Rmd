...
blas_pinned <- identical(Sys.getenv("OPENBLAS_NUM_THREADS", ""), "1")
if (grepl("openblas", sessionInfo()$BLAS, ignore.case = TRUE) && !blas_pinned)
  warning("OpenBLAS is multi-threaded and OPENBLAS_NUM_THREADS is not 1. The worker sweep ",
          "will oversubscribe this machine and speedups will be deflated. ",
...
data.frame(
  quantity = c("CPU", "physical cores", "logical CPUs", "threads per core", "NUMA nodes",
               "memory", "OS", "BLAS", "OpenBLAS pinned to 1 thread", "backend",
               "worker counts"),
