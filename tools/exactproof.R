...
norm_edgeR <- if (exists("normLibSizes", envir = asNamespace("edgeR"), inherits = FALSE)) {
  edgeR::normLibSizes
} else {
  edgeR::calcNormFactors
}
for (m in c("TMM","TMMwsp","RLE","upperquartile","none"))
  chk(paste("normLibSizes", m),
      identical(norm_edgeR(y, method = m), calcNormFactors_parallel(y, method = m, workers = 4L)))
chk("normLibSizes DGEList", identical(norm_edgeR(DGEList(y)), calcNormFactors_parallel(DGEList(y), workers = 4L)))
v <- voom(norm_edgeR(DGEList(y)), des)
...
ref_tt <- topTable(eBayes(contrasts.fit(lmFit(voom(norm_edgeR(DGEList(y)), des), des), cm)), number = Inf, sort.by = "none")
...
bch <- factor(rep(1:4, length.out = S))
rbe_ref <- removeBatchEffect(v$E, batch = bch)
chk("removeBatchEffect batch", identical(rbe_ref, removeBatchEffect_parallel(v$E, batch = bch, workers = 4L)))
chk("removeBatchEffect design", identical(removeBatchEffect(v$E, batch = bch, design = des),
     removeBatchEffect_parallel(v$E, batch = bch, design = des, workers = 4L)))
