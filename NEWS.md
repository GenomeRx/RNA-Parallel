...
- **The console progress tick no longer fires from inside a worker.** Two dispatch gates
  ...
  When that happens INSIDE a forked child on a recursive dispatch (ComBat-seq's tagwise
...
- **The orphan-fork exit check quit()d healthy workers.** The check that quits a worker
...
- **Integer overflow past 2^31 cells in three chunk-sizing/allocation sites.**
  `combat_row_chunks()`'s `combat.mem.chunk.cells` budget arithmetic
...
- **`glmFit_rows_parallel()`: same bind+reorder fusion as match_quantiles, with dimname
  ...
  base-R replacement-length error.
